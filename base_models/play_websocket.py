...
async def main():
    ...
    game_state = KoiKoiGameState(
        player_name=[your_name, ai_name], record_path=record_path, save_record=True
    )

    discard_model = torch.load(
        "model_agent/discard_rl_point.pt", map_location=torch.device("cpu")
    )
    pick_model = torch.load(
        "model_agent/pick_rl_point.pt", map_location=torch.device("cpu")
    )
    koikoi_model = torch.load(
        "model_agent/koikoi_rl_point.pt", map_location=torch.device("cpu")
    )

    ai_agent = AgentForTest(discard_model, pick_model, koikoi_model)

    async with websockets.serve(
        lambda websocket: game_loop(websocket, game_state, ai_agent), "localhost", 8765
    ):
    ...
    while True:
        if game_state.game_over:
            await websocket.send(
                json.dumps({"type": "game_over", "result": game_state.winner})
            )
            break
