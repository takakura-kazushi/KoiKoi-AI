...
def save_sample(record_name, game_state, result):
    ...
    sample = {
        "feature": game_state.feature_tensor,
        "result": result,
...
def round_replay_transform(round_num, init_point, round_record_dict, record_name):
    ...
    game_state.round_state.hand[1] = sorted(
        round_record_dict["basic"]["initHand1"].copy()
    )
    game_state.round_state.hand[2] = sorted(
        round_record_dict["basic"]["initHand2"].copy()
    )
    game_state.round_state.field_slot = (
        sorted(round_record_dict["basic"]["initBoard"].copy()) + [[0, 0]] * 10
    )
...
def record_dict_to_samples(record_dict, record_name):
    for round_num, point_1, point_2 in zip(*get_round_info(record_dict)):
        ...
        round_replay_transform(
            round_num, [point_1, point_2], round_record_dict, record_name
        )
    return
