...
class KoiKoiRoundStateBase:
    ...
    def __deal_card(self):
        ...
        while True:
            ...
            for suit in range(1, 13):
                if 4 in [
                    [card[0] for card in self.hand[1]].count(suit),
    ...
    def draw_pick(self, card=None):
        ...
        self.wait_action = (self.yaku_point(self.turn_player) > self.turn_point) and (
            self.turn_8 < 8
        )
    ...
    def yaku_point(self, player):
        ...
        # koikoi以外を抜き出すようにする
        yaku_point = sum(
            [yaku[2] for yaku in self.yaku(player) if yaku[1] != "Koi-Koi"]
        )
        koikoi_num = self.koikoi_num[player]
        if koikoi_num <= 3:
    ...
    def __call__(self, view=None):
        ...
        if self.state == "discard":
            ...
            print(
                "Round Point: You "
                + str(self.round_point[view])
                + ", Opponent "
                + str(self.round_point[op_view])
            )
        return
...
class KoiKoiGameStateBase:
    def __init__(
        self,
        round_num=1,
        round_total=DefaultVar.DEFAULT_ROUND_TOTAL,
    ...
    def new_round(self):
        ...
        if self.point[1] <= 0 or self.point[2] <= 0 or self.round == self.round_total:
            ...
            self.winner = (
                ...
                else (2 if self.point[1] < self.point[2] else 0)
            )
    ...
    def __game_result_record(self):
        ...
        self.log["info"]["endTime"] = time.strftime(
            "%Y-%m-%d-%H-%M-%S", time.localtime()
        )
        self.log["result"] = {
            "isOver": True,
            "gameWinner": self.winner,
            "player1EndPts": self.point[1],
            "player2EndPts": self.point[2],
        }
    ...
    def __save_record(self):
        ...
        filename = (
            ...
            + " "
            + self.log["info"]["player1Name"]
            + " vs "
            + self.log["info"]["player2Name"]
            + ".json"
        )
...
class KoiKoiRoundState(KoiKoiRoundStateBase):
    ...
    def __write_card_log_array(self, state):
        ...
        if state == "init":
            ...
            if self.pairing_card == []:
                self.card_log_dict[turn]["CardDiscardedAndUnpaired"] = np.array(
                    card_to_multi_hot(self.show)
                )
            else:
                self.card_log_dict[turn]["CardDiscardedAndPaired"] = np.array(
                    card_to_multi_hot(self.show)
                )

        elif state == "discard-pick":
            if self.collect != []:
                pair_by_discard_collect = np.array(
                    card_to_multi_hot(self.field_collect)
                )
                pair_by_discard = np.array(
                    card_to_multi_hot(self.log[f"turn{turn}"]["pairCard"])
                )
                self.card_log_dict[turn][
                    "CardPairedByDiscardCollect"
            ...
            if self.pairing_card == []:
                self.card_log_dict[turn]["CardDrawnAndUnpaired"] = np.array(
                    card_to_multi_hot(self.show)
                )
            else:
                self.card_log_dict[turn]["CardDrawnAndPaired"] = np.array(
                    card_to_multi_hot(self.show)
                )
            ...
            if self.collect != []:
                ...
                pair_by_discard = np.array(
                    card_to_multi_hot(self.log[f"turn{turn}"]["pairCard2"])
                )
                self.card_log_dict[turn][
                    "CardPairedByDrawnCollect"
                ] = pair_by_discard_collect
    ...
    def card_log_array(self):
        ...
        card_log_arrayについて
        """
        turn_list = [x for x in range(self.turn_16, 0, -1)] + [
            x for x in range(self.turn_16 + 1, 17)
        ]
        f_array = np.vstack(
            [f for turn in turn_list for _, f in self.card_log_dict[turn].items()]
        )
        return f_array
    ...
    def yaku_status_array(self):
        ...
        f_dict = {}
        f_dict["NumMyHand"] = [
            len(card_set & my_hand_card) for _, card_set in card_dict.items()
        ]
        f_dict["NumBoard"] = [
            len(card_set & board_card) for _, card_set in card_dict.items()
        ]
        f_dict["NumMyCollect"] = [
            len(card_set & my_collect_card) for _, card_set in card_dict.items()
        ]
        f_dict["NumOpCollect"] = [
            len(card_set & op_collect_card) for _, card_set in card_dict.items()
        ]
        f_dict["NumUnseen"] = [
            len(card_set & unseen_card) for _, card_set in card_dict.items()
        ]
        ...
        f_array_card_key = np.array(
            [card_to_multi_hot(card_set) for _, card_set in card_dict.items()]
        )
...
class KoiKoiGameState(KoiKoiGameStateBase):
    ...
    def game_status_array(self):
        ...
        f_dict["GamePoint"] = feature_tuple(
            float(point_diff) / 2, [0.5, 1, 1.5], [1, 0.5, 0.1]
        )
    ...
    def observation(self, view=None):
        ...
        observation["op_yaku"] = [
            [yaku[1], yaku[2]] for yaku in self.round_state.yaku(op_view)
        ]
        ...
        observation["your_yaku"] = [
            [yaku[1], yaku[2]] for yaku in self.round_state.yaku(view)
        ]
